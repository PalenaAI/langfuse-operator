...
func init() {
	...
	utilruntime.Must(langfusev1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&watchNamespaces, "watch-namespaces", "",
		"Comma-separated list of namespaces to watch. Overrides WATCH_NAMESPACE env var. "+
	...
	// Resolve watch namespaces: flag > env var > all namespaces
	if watchNamespaces == "" {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "e7849af6.palena.ai",
		Cache:                  cacheOptions,
	...
	if err := (&controller.LangfuseInstanceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "LangfuseInstance")
		os.Exit(1)
	}
	if err := (&controller.LangfuseOrganizationReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "LangfuseOrganization")
		os.Exit(1)
	}
	if err := (&controller.LangfuseProjectReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "LangfuseProject")
		os.Exit(1)
	}
	if err := (&controller.MigrationController{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
	...
	if err := (&controller.RetentionController{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&controller.SchemaDriftController{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "SchemaDrift")
		os.Exit(1)
	}
	if err := (&controller.CircuitBreakerController{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
